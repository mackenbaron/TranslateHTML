...
namespace htmlcxx {
	namespace HTML {
		...
		bool detect_utf8(const char *begin, int size)
		{
			...
			//const char *signature = "﻿\xEF\xBB\xBF";//utf-8的BOM
			//const char *signature = "﻿";//这里隐藏了一个文件DOM，是为\uFEFF
			//const char *signature = "";//手动输入，取消了此文件DOM
			const char *signature = "\xFE\xFF";//手动输入，取消了此文件DOM
			//string signature = "";
			...
			//if (!strncmp(begin, signature.c_str(), 3)) return true;
