...
namespace htmlcxx
{
	...
	namespace MyExtension{
		...
		std::vector<tree<HTML::Node>> GetSpecialNodeByTagProp(tree<HTML::Node> dom, std::vector<NodePropBase> ConditionLIB);
		//通过标签里的属性来确定相应节点，并将所有符合条件的节点显示出来
		void PrintSpecialNodeByTagProp(tree<HTML::Node> dom, std::vector<NodePropBase> ConditionLIB);
		...
		bool IsNeedTagNodeByNodeProp(tree<HTML::Node> dom, std::vector<MyExtension::NodePropBase> ConditionLIB, MatchedMode MatchFlag);
	}
